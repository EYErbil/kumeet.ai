...
class ActionItemCalendarEvent(BaseModel):
    action_item_id: str  # Keep as string in the model for flexibility
    title: str
    due_date: datetime
    calendar_type: Literal["google", "outlook"]
    user_id: Optional[str] = None

    # Add a property to get action_item_id as an integer when needed
    @property
    def action_item_id_int(self) -> Optional[int]:
        """Get action_item_id as an integer if possible."""
        if not self.action_item_id:
            return None
        try:
            return int(self.action_item_id)
