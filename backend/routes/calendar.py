...
async def create_action_item_event(action_item: ActionItemCalendarEvent, user_id: str = Depends(get_current_user)):
    ...
    try:
        logger.info(f"Creating action item event: {action_item.dict()}")
        ...
        logger.info(f"Found credentials for user {user_id} and calendar type {action_item.calendar_type}")
        logger.info(f"Credentials email: {getattr(credentials, 'email', 'Not set')}")
        logger.info(f"Access token exists: {bool(credentials.access_token)}")
        ...
        if not credentials:
            ...
            logger.warning(f"Token refresh failed for user {user_id} and calendar type {action_item.calendar_type}")
            auth_url = calendar_service.get_authorization_url(action_item.calendar_type)
            return {
                "message": f"Your {action_item.calendar_type} calendar connection has expired. Please reconnect.",
                "authorization_url": auth_url,
        ...
        try:
            # Create the event
            logger.info(f"Creating action item event in {action_item.calendar_type} calendar")
            calendar_event_id = calendar_service.create_action_item_event(credentials, action_item)
            logger.info(f"Successfully created action item event with ID: {calendar_event_id}")
            
            # Create calendar event object for database
            calendar_event = CalendarEvent(
                title=action_item.title,
                description=f"Action Item: {action_item.title}",
                start_time=action_item.due_date,
                end_time=action_item.due_date + timedelta(hours=1),  # Default to 1 hour duration
            ...
            # Save event to database
            event_id = calendar_repository.save_event(calendar_event)
            logger.info(f"Saved event to database with ID: {event_id}")
            
            return {
                "message": "Action item event created successfully", 
                "event_id": event_id,
                "status": "success"
            }
        except Exception as e:
            ...
            if "invalid_grant" in error_str:
                ...
                return {
                    ...
                    "help": "Go to Google Cloud Console > APIs & Services > Credentials and check your OAuth client configuration.",
        ...
        raise HTTPException(status_code=500, detail=f"Error creating action item event: {str(e)}")
