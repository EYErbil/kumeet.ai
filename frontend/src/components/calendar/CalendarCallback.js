        ...
        if (path.includes('/calendar/google/callback')) {
          ...
          try {
            ...
            if (response.data && response.data.status === 'success') {
              ...
              // Clean up the URL to prevent reloads from reusing the code
              window.history.replaceState({}, document.title, '/settings?completeGoogleAuth=true');
              ...
              localStorage.setItem('calendarError', JSON.stringify({
          ...
          setTimeout(() => navigate('/settings?completeGoogleAuth=true'), 3000);
          ...
          setTimeout(() => {
            setIsProcessing(false);
            navigate('/settings?completeOutlookAuth=true');
          }, 1000);
        } else {
