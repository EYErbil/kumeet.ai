  ...
  return (
    <div className="relative">
      ...
      {showDropdown && (
        <div 
          ref={dropdownRef}
          className={`absolute z-50 w-48 rounded-md shadow-lg bg-white dark:bg-gray-800 ring-1 ring-black ring-opacity-5 ${dropdownPosition === 'top' ? 'bottom-full mb-2' : 'top-full mt-2'} right-0`}
        >
      ...
      {/* Result Message */}
      {result && (
        <div className={`absolute z-50 right-0 ${dropdownPosition === 'top' ? 'bottom-full mb-2' : 'top-full mt-2'} p-2 rounded-md shadow-lg text-sm max-w-xs ${
          result.success 
          ...
          <div className="flex items-center">
            ...
            <span className="whitespace-normal break-words">{result.message}</span>
          </div>
        </div>
      )}
    </div>
