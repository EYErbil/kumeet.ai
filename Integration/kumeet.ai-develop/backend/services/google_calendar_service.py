...
class GoogleCalendarService:
    ...
    def create_action_item_event(self, credentials: GoogleCredentials, action_item: ActionItemCalendarEvent) -> str:
        ...
        try:
            ...
            # For all-day events in Google Calendar, the end date should be the next day
            next_day = (due_date + timedelta(days=1)).strftime('%Y-%m-%d')
            
            # Create event body for an all-day event
            event_body = {
                'summary': action_item.title,
                'description': f"Action Item: {action_item.title}",
                'start': {
                    'date': date_str,  # Use date format for all-day events
                },
                'end': {
                    'date': next_day,  # Next day for all-day event (Google's requirement)
                },
                'reminders': {
                    'useDefault': False,
                    'overrides': [
            ...
            try:
                created_event = service.events().insert(calendarId='primary', body=event_body).execute()
                logger.info(f"Created action item event with ID: {created_event.get('id')}")
                return created_event.get('id')
            except HttpError as e:
