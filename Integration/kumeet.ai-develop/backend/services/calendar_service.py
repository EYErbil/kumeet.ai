...
class CalendarService:
    ...
    def refresh_token_if_needed(self, credentials: CalendarCredentials) -> Optional[CalendarCredentials]:
        ...
        if not credentials.token_expiry:
            logger.info("No token expiry information, refreshing token")
            # No expiry info, so refresh to be safe
            try:
                if credentials.calendar_type == "google":
                    return self.google_service.refresh_token(credentials)
                elif credentials.calendar_type == "outlook":
                    return self.outlook_service.refresh_token(credentials)
            except Exception as e:
                logger.error(f"Error refreshing token without expiry: {str(e)}")
                return None
