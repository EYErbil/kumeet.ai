...
class NotesService:
    ...
    def create_note(note_data):
        ...
        try:
            ...
            if 'content' not in note_data or not note_data['content']:
                raise ValueError("Note content is required")

            # Convert meeting ID to integer if provided
            meeting_id = None
            if 'meeting_id' in note_data and note_data['meeting_id']:
                try:
                    meeting_id = int(note_data['meeting_id'])
                except (ValueError, TypeError):
                    logger.warning(f"Invalid meeting ID format: {note_data['meeting_id']}")
    ...
    def update_note(note_id, note_data):
        ...
        """
        try:
            # Convert note_id to integer if it's a string
            ...
            # Extract data from note_data
            updates = {}
            
            # Note content
            if 'content' in note_data and note_data['content'] is not None:
                updates['note_text'] = note_data['content']
            
            # Meeting ID
            if 'meeting_id' in note_data and note_data['meeting_id'] is not None:
                try:
                    updates['meeting_id'] = int(note_data['meeting_id'])
                except (ValueError, TypeError):
                    logger.warning(f"Invalid meeting ID format: {note_data['meeting_id']}, skipping update")
            ...
            with transaction() as conn:
                with conn.cursor(cursor_factory=RealDictCursor) as cur:
                    ...
                    if updates.get('meeting_id') and updates['meeting_id'] != existing_note['meeting_id']:
                        cur.execute("""
                            SELECT title, meeting_date
                            FROM meetings
                            WHERE meeting_id = %s
                    ...
                    updated_note = {
                        'id': str(note_id),
                        'content': updates.get('note_text') or existing_note['note_text'],
                        'createdBy': {
